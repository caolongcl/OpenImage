...
namespace clt {
  struct PolygonMsg : public Msg {
    PolygonMsg(const std::string &_target, const std::string &_what,
               const std::shared_ptr<MsgData> &_data = nullptr) :
      Msg(Type::MSG_POLYGON, _target, _what, _data) {}
  };

  struct TextMsg : public Msg {
    TextMsg(const std::string &_target, const std::string &_what,
            const std::shared_ptr<MsgData> &_data = nullptr) :
  ...
  struct BaseModelMsg : public Msg {
    BaseModelMsg(const std::string &_target, const std::string &_what,
                 const std::shared_ptr<MsgData> &_data = nullptr)
      :
