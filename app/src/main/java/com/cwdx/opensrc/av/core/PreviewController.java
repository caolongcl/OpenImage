...
import com.cwdx.opensrc.av.utils.ImageUtils;
...
//      Utils.d(TAG, "DeInit");
  ...
  public synchronized void SelectCamera(CameraSelector selector) {
    ...
    Utils.d(TAG, "SelectCamera " + mSelector);
  }
...
//    boolean hasCopyRes = false;
    Utils.d(TAG, "res has copied " + hasCopyRes);
    if (!hasCopyRes) {
      Utils.d(TAG, "copy res... ");
  ...
  private void FromNativeConfigEncoder(boolean start, int width, int height, int fps, int bitrate) {
    Utils.d(TAG, "start " + start + " wh(" + width + " " + height + " fps " + fps + " bitrate " + bitrate);
    if (start) {
  ...
  private void FromNativeSetPreviewTexture(int texId) {
    Utils.d(TAG, "FromNativeSetPreviewTexture id:" + texId);
  ...
  private void FromNativeStartPreview() {
    Utils.d(TAG, "FromNativeStartPreview");
...
//      Utils.d(TAG, "FromNativeUpdateFrame");
  ...
  private void FromNativeStopPreview() {
    Utils.d(TAG, "FromNativeStopPreview");
  ...
  private void FromNativeCreateImageReader(int width, int height) {
    Utils.d(TAG, "FromNativeCreateImageReader " + width + " " + height);
  ...
  private void handleImage(Image image) {
    ...
    Utils.d(TAG, "image row padding:" + rowPadding);
