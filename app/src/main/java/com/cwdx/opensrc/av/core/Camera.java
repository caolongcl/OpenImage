...
public class Camera implements ICamera {
  ...
  private CameraManager.AvailabilityCallback mAvailabilityCallback = new CameraManager.AvailabilityCallback() {
    ...
    public void onCameraAvailable(@NonNull String cameraId) {
      Utils.v(TAG, cameraId + " available");
    }

    @Override
    public void onCameraUnavailable(@NonNull String cameraId) {
      Utils.v(TAG, cameraId + " unavailable");
    }

    @Override
    public void onCameraAccessPrioritiesChanged() {
      Utils.v(TAG, "accessPrioritiesChanged");
    }
  };
  ...
  private CameraManager.TorchCallback mTorchCallback = new CameraManager.TorchCallback() {
    ...
    public void onTorchModeUnavailable(@NonNull String cameraId) {
      Utils.v(TAG, cameraId + " TorchModeUnavailable");
    }

    @Override
    public void onTorchModeChanged(@NonNull String cameraId, boolean enabled) {
      Utils.v(TAG, cameraId + " TorchMode " + enabled);
    }
  };
  /**
   * 以下为内部实现
  ...
  private int parseSurfaceRotation(int displayOrientation) {
    ...
    switch (displayOrientation) {
      ...
      case Surface.ROTATION_270:
        ...
        break;
      default:
        Utils.w(TAG, "invalid surfaceRotation " + displayOrientation);
