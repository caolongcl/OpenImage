...
struct compute_rgbToSrgb {
  ...
  call(vecType<T, P> const& ColorRGB, T GammaCorrection)
  {
    ...
    return mix(
      pow(ClampedColor, vecType < T, P > (GammaCorrection)) * static_cast<T>(1.055)
        - static_cast<T>(0.055),
...
struct compute_rgbToSrgb<T, P, tvec4> {
  ...
  {
    return tvec4 < T, P
      > (compute_rgbToSrgb<T, P, tvec3>::call(tvec3<T, P>(ColorRGB), GammaCorrection), ColorRGB.a);
  }
};
...
struct compute_srgbToRgb<T, P, tvec4> {
  ...
  {
    return tvec4 < T, P
      > (compute_srgbToRgb<T, P, tvec3>::call(tvec3<T, P>(ColorSRGB), Gamma), ColorSRGB.a);
  }
};
}//namespace detail
