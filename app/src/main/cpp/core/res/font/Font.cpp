 ...
 */
void Font::renderText(const TextInfo &info) {
    // position 都是窗口坐标系的，需要换算到实际绘制字体的坐标系位置
    Float2 position{info.position.x * ((float) info.realViewport.width / (float) info.displayViewport.width),
                    info.position.y * ((float) info.realViewport.height / (float) info.displayViewport.height)};

    float scale = info.scale * ((float) info.realViewport.width / (float) cTextCountPerWidth) /
    ...
    // 换到 OpenGL 坐标系
    position.x = position.x + (float) info.realViewport.x;
    position.y = -position.y + (float) info.realViewport.y + (float) info.realViewport.height;
...
//    Log::e(Log::RENDER_TAG, "info.rotate %f", info.rotate);
    ...
    glm::mat4 projection = glm::ortho((float) info.realViewport.x,
                                      (float) info.realViewport.x + (float) info.realViewport.width,
                                      (float) info.realViewport.y,
                                      (float) info.realViewport.y + (float) info.realViewport.height);
    // 绘制字体背景
